...
- [CI] UI smoke test の偽陽性 assert(メイン画面のボタンラベルにも一致)を排除し、シートの close ボタン出現を待つリトライ方式に変更。CI の既存 failure の修正。
- 実機確認が必要: ドラッグ&ドロップ(Finder でのファイル移動)、ホバーでのツールチップ表示、JIS キーボードでの記号ショートカット。
